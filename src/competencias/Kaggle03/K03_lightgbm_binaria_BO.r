...
#Mis Semillas: 106703, 106721, 106727, 106739, 106747
...
EstimarGanancia_lightgbm <- function(x) {
  ...
  cat(format(Sys.time(), format = "%Y-%m-%d %H:%M:%S"), "EstimarGanancia_lightgbm iteracion", GLOBAL_iteracion, "\n")
  ...
  if (ganancia_test_normalizada > GLOBAL_gananciamax) {
    ...
    fwrite(tb_importancia,
      file = paste0(format(inicio, format = "%Y-%m-%d %H:%M:%S"), "impo_", sprintf("%03d", GLOBAL_iteracion), ".txt"),
      sep = "\t"
    )
  ...
  loguear(xx, arch = paste0(format(inicio, format = "%Y-%m-%d %H:%M:%S"), "-BO_log.txt"))
